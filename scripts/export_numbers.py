...
Stage-1: per-component metrics are computed from each pkl's `oof_per_model`
(the `prl` column in sl_cv_metrics is the *ensemble* value broadcast to every
row, so it is NOT used); ensemble metrics + AUC-PR + the PRL decomposition come
from sl_oof; PCA/feature counts + NNLS weights from the pkls; burnout from the
...
Run:  .venv/bin/python scripts/export_numbers.py   (loads 25 pkls; ~2 min)
"""
...
# ======================================================================
# STAGE 1 -- measurement model (from pkls' oof_per_model + sl_oof + notebook)
# ======================================================================
...
ens, comp, pca = [], [], {"X": [], "W": [], "XW": []}
feat = {}
for f in sorted(glob.glob(str(INTERIM / "sl_model_*.pkl"))):
    draw = re.search(r"sl_model_(\d_\d)\.pkl", f).group(1)
    r = joblib.load(f)
    oof = pd.read_parquet(f.replace("sl_model", "sl_oof").replace(".pkl", ".parquet"))
    ...
    for (mode, name), probs in r["oof_per_model"].items():
        comp.append(dict(draw=draw, mode=mode, learner=name,
                         ...
                         wt=r["weights"].get((mode, name), 0.0) / wsum))
    for m in ("X", "W", "XW"):
        pca[m].append(int(r["pipelines"][m]["pca"].n_components_)); feat[m] = len(r["pipelines"][m]["feat_cols"])
    del r
